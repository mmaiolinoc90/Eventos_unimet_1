...
<head>
    ...
    <title>Comprobante Evento Externo</title>
                ...
                <ul class="nav navbar-nav navbar-right">
                    ...
                    <li>
                       	 <?php //inicio PHP	
								...
								$id_usuario = mysql_query('SELECT * FROM evento_externo WHERE id_evento_externo="'.mysql_real_escape_string($id_evento).'"');
								
								while ($row = mysql_fetch_row($id_usuario)){
									
									$id_usuario = mysql_query('SELECT * FROM usuario_externo WHERE id_usuario_externo="'.mysql_real_escape_string($row[8]).'"');
        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-12 text-center">
                    <h2 class="section-heading">Comprobante Evento Externo</h2>
            ...
            <div class="row">
               <!-- datos personales -->
                <div class="col-lg-12"> 
                    <form name="formulario_registro" action="index.php" novalidate>
                        <div class="row">    
                            <div class="col-md-6">
                              <h3 style="margin-left: 20%; color: white; font-size: 30px; font-weight: bold;">Datos Personales</h3>
                               ...
                               <?php //inicio PHP	
                    			//se envia la consulta  
								//se envia la consulta  
								$id_evento_exter = mysql_query('SELECT * FROM evento_externo WHERE id_evento_externo="'.mysql_real_escape_string($id_evento).'"');
								
								while ($row3 = mysql_fetch_row($id_evento_exter)){
									
									$usu = mysql_query('SELECT * FROM usuario_externo WHERE id_usuario_externo="'.mysql_real_escape_string($row3[8]).'"');
									
									while ($row4 = mysql_fetch_row($usu)){
								
                                echo '<div class="form-group">'; 
                                    echo '<input type="text" value="Nombre del Solicitante: '.$row4[1].'" class="form-control" disabled id="name" name="nombre">';
                                   echo '<p class="help-block text-danger"></p>';
                               echo '</div>';
                                echo '<div class="form-group">';
                                    echo '<input type="text" value="Apellido del Solicitante: '.$row4[2].'" class="form-control" disabled name="apellido" id="apellido">';
                                   echo '<p class="help-block text-danger"></p>';
                               echo '</div>';
									echo '<div class="form-group">';
                                    echo '<input type="text" value="Rif del solicitante: '.$row4[3].'" class="form-control" disabled name="cedula" id="cedula">';
                                   echo '<p class="help-block text-danger"></p>';
                               echo '</div>';
                                echo '<div class="form-group">';
                                   echo '<input type="email" value="Correo electronico del solicitante: '.$row4[4].'" disabled class="form-control" id="email" name="email">';
                                    echo '<p class="help-block text-danger"></p>';
                                echo '</div>';
								echo '<div class="form-group">';
                                	echo '<input type="email" value="Telefono del solicitante: '.$row4[5].'" disabled class="form-control" id="telefono" name="telefono">';
                                    echo '<p class="help-block text-danger"></p>';
                                echo '</div>';
								echo '<div class="form-group">';
                                	echo '<input type="email" value="Nombre de la empresa: '.$row4[6].'" disabled class="form-control" id="empresa" name="empresa">';
                                    echo '<p class="help-block text-danger"></p>';
                                echo '</div>';
								echo '<div class="form-group">';
                                	echo '<input type="email" value="Cargo del Solicitante: '.$row4[7].'" disabled class="form-control" id="cargo" name="cargo">';
                                    echo '<p class="help-block text-danger"></p>';
                                echo '</div>';
                            ...
                            <div class="col-md-6">
                               ...
                               echo  '</div>';
							   echo '<div class="form-group">';
                                    echo '<input type="text"  class="form-control" value="  '.$row_datos_evento[7].' se requiere entradas" name="presupuesto" disabled>  ';
                                  echo '<p class="help-block text-danger"></p>';
                               echo '</div>';
							   echo '<div class="form-group">';
                                    echo '<input type="text"  class="form-control" value="Status del evento:   '.$row_datos_evento[5].'" name="no_aprobado" disabled>  ';
                                  echo '<p class="help-block text-danger"></p>';
                               echo '</div>';
	...
	<!-- FIn Sección comprobante datos personales y evento -->
				  ...
				  <!-- Sección datos de la sesion -->
					<section id="contact" style="background-repeat: repeat; background-position:top;">
						...
						<?php 
						   //se envia la consulta  
							$sesiones = mysql_query('SELECT * FROM agenda_externa WHERE id_evento_externo="'.mysql_real_escape_string($id_evento).'"');
                	...
                	<div class="col-lg-12">
                        <div class="row">
                            ...
                            <div class="col-md-6" style=" height: 430px; overflow: auto;">
                                ...
                                <div class="form-group">  			  
									<input style="height: 147px;" class="form-control" value="Descripción de la sesión: <?php echo $row_sesiones[11] ?>"   id="descripcion" name="descripcion" disabled>';
                                <p class="help-block text-danger"></p>
                                </div>  					
